...
export function FluxusManagerGuidance({ result }: { result: FluxusResult }) {
  ...
  const followUp = result.sustentabilidade;
  return (
    <div className="space-y-5">
      <section className="rounded-3xl border border-border/60 bg-card p-6">
        ...
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">Perfil predominante: <strong className="text-foreground">{FLUXUS_DIMENSION_META[result.predominante].label}</strong>. Maior demanda de adaptação: <strong className="text-foreground">{meta.label}</strong> ({demand.demanda >= 0 ? "+" : ""}{demand.demanda.toFixed(1)}). A pressão média de adaptação está na faixa {pressureBand.toLowerCase()} e o sinal exploratório recomenda: {followUp.toLowerCase()}. Confirme cada hipótese com o participante e com evidências reais do trabalho.</p>
      </section>
      <Card className="border-border/60"><CardHeader><CardTitle className="text-base">Como atuar em cada dimensão</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">{FLUXUS_DIMENSIONS.map(dimension => { const meta = FLUXUS_DIMENSION_META[dimension]; const item = result.dimensions[dimension]; const itemGuidance = guidance[dimension]; return <div key={dimension} className="rounded-2xl border border-border/60 p-5"><div className="flex items-center justify-between gap-3"><h3 className="font-semibold">{meta.label}</h3><Badge variant="secondary">Natural {item.natural.toFixed(1)}</Badge></div><p className="mt-3 text-sm"><strong>Como atuar:</strong> {itemGuidance.act}</p><p className="mt-3 text-sm text-muted-foreground"><strong className="text-foreground">Evitar:</strong> {itemGuidance.avoid}</p><p className="mt-3 text-sm text-muted-foreground"><strong className="text-foreground">Condição de apoio a explorar:</strong> {itemGuidance.motivate}</p></div>; })}</CardContent></Card>
      ...
      <Card className="border-border/60"><CardHeader><CardTitle className="text-base">Indicadores para perguntas de acompanhamento</CardTitle></CardHeader><CardContent className="grid gap-3 sm:grid-cols-2"><Indicator label="Energia autorrelatada" value={result.energia.score} band={result.energia.faixa} /><Indicator label="Autorregulação autorrelatada" value={result.autorregulacao.score} band={result.autorregulacao.faixa} /><Indicator label="Distância média de adaptação" value={result.distanciaMediaAdaptacao} band={pressureBand} /><Indicator label="Maior distância observada" value={result.maiorGap} band={followUp} /></CardContent></Card>
      ...
      <aside className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5 text-sm leading-relaxed text-muted-foreground"><strong className="text-foreground">Nota de método.</strong> Este relatório orienta uma conversa de desenvolvimento e não deve fundamentar isoladamente decisões sobre pessoas. As leituras são hipóteses comportamentais; não medem caráter, inteligência, saúde ou competência técnica.</aside>
    </div>
  );
}
